...
export async function PUT(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const saved = await prisma.$transaction(async (tx) => {
      const point = await tx.knowledgePoint.findFirst({ where: { id, courseId: RADIO_COURSE_ID } });
      if (!point) throw new ApiError("知识点不存在", 404);
      const changed = await tx.knowledgePoint.updateMany({ where: { id, courseId: RADIO_COURSE_ID, version: input.version }, data: { name: input.name, sortOrder: input.sortOrder, enabled: input.enabled, version: { increment: 1 } } });
      if (changed.count !== 1) throw new ApiError(STALE_VERSION_MESSAGE, 409);
      await tx.knowledgePoint.updateMany({ where: { courseId: RADIO_COURSE_ID, path: { startsWith: `${point.path}/` } }, data: { enabled: input.enabled, version: { increment: 1 } } });
      const updated = await tx.knowledgePoint.findFirstOrThrow({ where: { id, courseId: RADIO_COURSE_ID } });
