...
export async function PUT(request: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const [question, level, point] = await Promise.all([
      prisma.question.findFirst({ where: { id } }),
    ...
    const saved = await prisma.$transaction(async (tx) => {
      if (input.externalQuestionCode) {
        const duplicate = await tx.question.findFirst({ where: { id: { not: id }, levelId: input.levelId, externalQuestionCode: input.externalQuestionCode } });
