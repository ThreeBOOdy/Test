...
export async function GET(_: Request, context: { params: Promise<{ id: string }> }) {
  try {
    ...
    const question = await prisma.question.findFirst({ where: { id }, select: { id: true } });
    if (!question) throw new ApiError("题目不存在", 404);
    const revisions = await prisma.questionRevision.findMany({ where: { questionId: id }, include: { actor: { select: { displayName: true } } }, orderBy: { revision: "desc" } });
