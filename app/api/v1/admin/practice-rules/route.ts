...
export async function PUT(request: Request) {
  try {
    ...
    await prisma.$transaction(async (tx) => {
      for (const rule of input.levelRules) await tx.levelPracticeRule.upsert({ where: { courseId_levelId: { courseId: RADIO_COURSE_ID, levelId: rule.levelId } }, update: { singleCount: rule.singleCount, multipleCount: rule.multipleCount, enabled: true }, create: { ...rule, courseId: RADIO_COURSE_ID, enabled: true } });
      for (const rule of input.knowledgeRules) {
        if (rule.singleCount === 0 && rule.multipleCount === 0) await tx.knowledgePracticeRule.deleteMany({ where: { courseId: RADIO_COURSE_ID, knowledgePointId: rule.knowledgePointId, levelId: rule.levelId } });
        else await tx.knowledgePracticeRule.upsert({ where: { courseId_knowledgePointId_levelId: { courseId: RADIO_COURSE_ID, knowledgePointId: rule.knowledgePointId, levelId: rule.levelId } }, update: { singleCount: rule.singleCount, multipleCount: rule.multipleCount, enabled: true }, create: { ...rule, courseId: RADIO_COURSE_ID, enabled: true } });
      }
      for (const rule of input.examRules) await tx.examRule.upsert({ where: { courseId_levelId: { courseId: RADIO_COURSE_ID, levelId: rule.levelId } }, update: { singleCount: rule.singleCount, multipleCount: rule.multipleCount, durationMinutes: rule.durationMinutes, passingCount: rule.passingCount, enabled: true }, create: { ...rule, courseId: RADIO_COURSE_ID, enabled: true } });
    });
