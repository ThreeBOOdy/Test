...
export async function POST(request: Request) {
  try { assertSameOrigin(request); const admin = await requireAdministrator(); const { ids } = schema.parse(await readJsonBody(request)); const today = getBusinessDate(); for (const id of ids) await approveRegistration(admin.id, id, {}, today); return NextResponse.json({ approved: ids.length }); }
