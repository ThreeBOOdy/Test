...
export async function POST(request: Request) {
  try { assertSameOrigin(request); const reviewer = await requireTeachingUser(); const { ids } = schema.parse(await readJsonBody(request)); const today = getBusinessDate(); for (const id of ids) await approveRegistration(reviewer.id, id, {}, today); return NextResponse.json({ approved: ids.length }); }
