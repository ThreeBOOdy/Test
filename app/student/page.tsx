...
export default async function StudentPage() {
  ...
  const [sessions, wrongQuestions, levelRules, knowledgeRules, activeQuestions, focusOverview, reviewPlan, playerStatus, gamificationVisibility] = await Promise.all([
  ...
  const availableLevels = levelRules.filter((rule) => { const pool = activeQuestions.filter((question) => question.levels.some((item) => item.levelId === rule.levelId)); const singles = pool.filter((question) => question.type === "SINGLE_CHOICE").length; return singles >= rule.singleCount && pool.length - singles >= rule.multipleCount && rule.singleCount + rule.multipleCount > 0; });
  const availableKnowledge = knowledgeRules.filter((rule) => { const pool = activeQuestions.filter((question) => question.levels.some((item) => item.levelId === rule.levelId) && (question.knowledgePoint.path === rule.knowledgePoint.path || question.knowledgePoint.path.startsWith(`${rule.knowledgePoint.path}/`))); const singles = pool.filter((question) => question.type === "SINGLE_CHOICE").length; return singles >= rule.singleCount && pool.length - singles >= rule.multipleCount && rule.singleCount + rule.multipleCount > 0; }).slice(0, 6);
  const primaryHref = activeSession ? `/student/practice?session=${activeSession.id}` : "/student/practice/start";
  const primaryTitle = activeSession ? "继续上次练习" : "开始新的练习";
  const primaryDescription = activeSession ? "从上次中断的位置继续作答，已选答案和训练进度都会保留。" : "从综合、顺序、随机、专项或模拟考试中挑选一种，随时开始。";
  ...
  return <AppShell role="student" currentPath="/student"><div className="safe-bottom"><PageHeader title={`欢迎回来，${user.displayName}`} description="训练频道、学习进度和错题都会实时同步；继续练习将自动回到上次的位置。" eyebrow="PERSONAL SIGNAL DESK" />
    ...
    <section className="mt-8"><div className="mb-4 flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between"><div><div className="text-xs font-bold text-[var(--primary)]">TRAINING CHANNELS</div><h2 className="mt-1 text-xl font-extrabold">选择训练频道</h2></div><Link href="/student/practice/start" className="text-sm font-bold text-[var(--primary)]">查看全部练习</Link></div>{availableLevels.length || availableKnowledge.length ? <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{availableLevels.map((rule) => <ChannelCard key={rule.id} href={`/student/practice/start?mode=level&level=${rule.level.code}`} icon={Layers3} title={`${rule.level.code}级综合训练`} description="覆盖本等级全部知识点，适合系统化检测。" meta={`单选 ${rule.singleCount} · 多选 ${rule.multipleCount}`} />)}{availableKnowledge.map((rule) => <ChannelCard key={rule.id} href={`/student/practice/start?mode=knowledge&level=${rule.level.code}&knowledge=${rule.knowledgePoint.id}`} icon={Target} title={rule.knowledgePoint.name} description="集中攻克单一知识点，练得更准、更快。" meta={`${rule.level.code}级 · ${rule.singleCount + rule.multipleCount} 题`} />)}</div> : <Card><EmptySignalState title="当前没有可用训练频道" description="教师补充题目后，新的练习就会出现在这里。" /></Card>}</section>
...
function ChannelCard({ href, icon: Icon, title, description, meta }: { href: string; icon: LucideIcon; title: string; description: string; meta: string }) { return <Link href={href as never} className="group"><Card className="h-full transition-[border-color,transform] duration-200 hover:-translate-y-0.5 hover:border-[var(--border-strong)]"><CardContent><div className="flex items-start justify-between"><div className="grid size-11 place-items-center rounded-2xl border border-cyan-600/15 bg-cyan-500/10 text-[var(--primary)]"><Icon className="size-5" /></div><ArrowRight className="size-4 text-[var(--muted-foreground)] transition-transform group-hover:translate-x-1" /></div><h3 className="mt-5 text-lg font-extrabold">{title}</h3><p className="mt-2 text-sm leading-7 text-[var(--muted-foreground)]">{description}</p><Badge className="mt-5" tone="blue">{meta}</Badge></CardContent></Card></Link>; }
function practiceTitle(mode: string, levelCode?: string, knowledgeName?: string) { if (mode === "WRONG_QUESTION") return "错题巩固练习"; if (mode === "KNOWLEDGE_POINT") return `${knowledgeName ?? "知识点专项"} · ${levelCode ?? "-"}级`; if (mode === "QUESTION_ORDER") return `${levelCode ?? "-"}级顺序练习`; if (mode === "RANDOM_ALL") return `${levelCode ?? "-"}级智能随机练习`; if (mode === "MOCK_EXAM") return `${levelCode ?? "-"}级模拟考试`; return `${levelCode ?? "-"}级综合练习`; }
