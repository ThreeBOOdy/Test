...
export default async function TeacherPage() {
  ...
  const [levels, activeQuestions, knowledgeCount, studentCount, recentSessions, importCount] = await Promise.all([
    ...
    prisma.question.findMany({ where: { status: "ACTIVE" }, select: { levels: { select: { levelId: true } }, type: true } }),
  ...
  const inventories = levels.map((level) => {
    const items = activeQuestions.filter((question) => question.levels.some((item) => item.levelId === level.id));
  ...
  return (
    <AppShell role="teacher" currentPath="/teacher">
      <div className="safe-bottom">
        <section className="on-dark receiver-panel instrument-grid relative overflow-hidden rounded-[30px] border-transparent bg-[#07121b] p-6 text-white sm:p-8">
        ...
        <div className="mt-7 grid gap-6 xl:grid-cols-[1.18fr_.82fr]">
          <Card className="overflow-hidden"><CardHeader className="border-b border-[var(--border)] pb-5"><div className="flex items-center justify-between gap-4"><div><div className="text-[10px] font-black tracking-[0.2em] text-[var(--primary)]">INVENTORY SPECTRUM</div><CardTitle className="mt-1">各等级题库库存</CardTitle><CardDescription>颜色条展示单选与多选的相对容量</CardDescription></div><Link href="/teacher/rules" className="flex items-center gap-2 text-sm font-extrabold text-[var(--primary)]">配置规则<ArrowRight className="size-4" /></Link></div></CardHeader><CardContent className="flex flex-col gap-4">{inventories.map(({ level, singles, multiples, total }) => <div key={level.id} className="rounded-[20px] border border-[var(--border)] bg-[var(--muted)]/65 p-4 transition hover:border-cyan-400 hover:bg-cyan-50/60"><div className="flex items-center gap-3"><div className="grid size-11 place-items-center rounded-xl bg-[var(--ink)] font-black text-cyan-200 shadow-lg">{level.code}</div><div><div className="font-black">{level.code}级</div><div className="mt-0.5 text-xs text-[var(--muted-foreground)]">{total} 道启用题目</div></div><div className="ml-auto flex gap-2"><Inventory label="单选" value={singles} /><Inventory label="多选" value={multiples} /></div></div><div className="mt-4 flex h-2.5 overflow-hidden rounded-full bg-slate-200"><div className="bg-[linear-gradient(90deg,#0a96a2,#28c5cb)] transition-all" style={{ width: `${(singles / maxInventory) * 100}%` }} /><div className="bg-[linear-gradient(90deg,#e9a341,#ffc86d)] transition-all" style={{ width: `${(multiples / maxInventory) * 100}%` }} /></div></div>)}</CardContent></Card>

          <div className="flex flex-col gap-6">
            <Card><CardHeader><div className="text-[10px] font-black tracking-[0.2em] text-[var(--primary)]">SYSTEM SIGNALS</div><CardTitle>系统状态</CardTitle><CardDescription>题库与练习功能的运行情况</CardDescription></CardHeader><CardContent className="flex flex-col gap-3"><Notice icon={Layers3} tone="green" title="综合与专项练习已就绪" description="等级综合与知识点专项训练正常开放" /><Notice icon={FileSpreadsheet} tone="blue" title={`${importCount} 个已提交导入批次`} description="表格预检与批量导入已启用" /><Notice icon={AlertCircle} tone="amber" title="AI 解析即将上线" description="上线后将先经过教师审核" /></CardContent></Card>
            <Card className="ink-card overflow-hidden bg-[var(--ink)] text-white"><CardContent className="relative"><div className="absolute -right-12 -top-12 size-40 rounded-full bg-cyan-300/10 blur-2xl" /><div className="relative flex items-start gap-4"><div className="radio-waves grid size-12 shrink-0 place-items-center rounded-2xl bg-cyan-300/10 text-cyan-300"><RadioTower className="relative z-10 size-6" /></div><div><div className="text-[10px] font-black tracking-[0.18em] text-cyan-100/50">NEXT ACTION</div><h3 className="mt-1 text-lg font-black">保持题库信号稳定</h3><p className="mt-2 text-sm leading-6 text-slate-400">优先补充题目不足的等级与知识点，再向学生开放练习入口。</p><Link href="/teacher/rules" className="mt-5 inline-flex items-center gap-2 text-sm font-extrabold text-cyan-300">检查抽题规则<ArrowRight className="size-4" /></Link></div></div></CardContent></Card>
          </div>
        </div>
      </div>
    </AppShell>
