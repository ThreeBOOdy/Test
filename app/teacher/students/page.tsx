...
type StudentStats = { userId: string; sessionCount: number | bigint | string; answered: number | bigint | string; correct: number | bigint | string; lastActive: Date | null };
...
export default async function StudentsPage({ searchParams }: { searchParams: Promise<{ page?: string; search?: string }> }) {
  ...
  const where = { role: "STUDENT" as const, ...(params.search ? { OR: [{ username: { contains: params.search } }, { displayName: { contains: params.search } }] } : {}) };
  ...
  const stats = ids.length ? await prisma.$queryRaw<StudentStats[]>(Prisma.sql`SELECT ps.\`userId\`, CAST(COUNT(DISTINCT ps.id) AS SIGNED) AS \`sessionCount\`, CAST(COUNT(pa.id) AS SIGNED) AS answered, CAST(SUM(CASE WHEN pa.\`isCorrect\` = TRUE THEN 1 ELSE 0 END) AS SIGNED) AS correct, MAX(ps.\`startedAt\`) AS \`lastActive\` FROM \`PracticeSession\` ps LEFT JOIN \`PracticeAnswer\` pa ON pa.\`sessionId\` = ps.id WHERE ps.\`userId\` IN (${Prisma.join(ids)}) GROUP BY ps.\`userId\``) : [];
  const statsByUser = new Map(stats.map((item) => [item.userId, { ...item, sessionCount: Number(item.sessionCount), answered: Number(item.answered), correct: Number(item.correct) }]));
  const rows = students.map((student) => { const item = statsByUser.get(student.id); return { id: student.id, username: student.username, displayName: student.displayName, enabled: student.enabled, mustChangePassword: student.mustChangePassword, sessionCount: item?.sessionCount ?? 0, accuracy: item?.answered ? Math.round(item.correct / item.answered * 100) : 0, lastActive: item?.lastActive?.toLocaleString("zh-CN") ?? "尚未练习" }; });
