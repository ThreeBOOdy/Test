...
export default async function KnowledgePage() {
  ...
  return <AppShell role="teacher" currentPath="/teacher/knowledge"><div className="safe-bottom"><PageHeader title="知识点目录" description={`当前共有 ${points.length} 个知识点。新增分类号时会自动补齐缺失的父级目录。`} /><KnowledgeManager points={points.map((point) => ({ id: point.id, parentId: point.parentId, code: point.code, name: point.name, depth: point.depth, sortOrder: point.sortOrder, enabled: point.enabled, version: point.version, childCount: point._count.children, questionCount: point._count.questions }))} /></div></AppShell>;
}
