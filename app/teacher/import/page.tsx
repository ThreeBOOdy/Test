...
export default async function ImportPage() {
  ...
  const batches = await prisma.importBatch.findMany({ where: { courseId: RADIO_COURSE_ID, importedById: user.id }, orderBy: { createdAt: "desc" }, take: 20, select: { id: true, fileName: true, status: true, totalRows: true, insertedRows: true, duplicateRows: true, warningRows: true, errorRows: true, createdAt: true } });
