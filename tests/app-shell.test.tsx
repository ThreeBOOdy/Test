...
describe("application shell", () => {
  ...
  it("makes every teacher section reachable on mobile", async () => {
    ...
    for (const label of ["管理概览", "题库管理", "字母类维护", "学生管理", "知识点类型维护", "抽题规则", "题库导入", "教学统计"]) expect(within(dialog).getByRole("link", { name: label })).toBeInTheDocument();
    for (const label of ["注册审核", "学生账号", "学生导入", "年级配置", "人物身份目录"]) expect(within(dialog).queryByRole("link", { name: label })).not.toBeInTheDocument();
  });
  ...
  it("keeps administrator account tools out of the teacher desktop navigation", () => {
    ...
    for (const label of ["注册审核", "学生账号", "学生导入", "年级配置", "人物身份目录"]) expect(screen.queryByRole("link", { name: label })).not.toBeInTheDocument();
  });
  ...
  it("shows only administrator account tools in the administrator console", () => {
    ...
    for (const label of ["教学控制台", "题库管理", "知识点类型维护", "抽题规则", "题库导入", "教学统计"]) expect(screen.queryByRole("link", { name: label })).not.toBeInTheDocument();
  });
