...
export async function ensureKnowledgePoint(
  ...
  for (let index = 0; index < segments.length; index += 1) {
    ...
    const existing = await tx.knowledgePoint.findUnique({ where: { courseId_code: { courseId: RADIO_COURSE_ID, code: currentCode } }, include: { _count: { select: { questions: true } } } });
    ...
    current = await tx.knowledgePoint.upsert({
      where: { courseId_code: { courseId: RADIO_COURSE_ID, code: currentCode } },
