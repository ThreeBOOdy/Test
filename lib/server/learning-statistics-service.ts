...
export async function getStudentLearningSummary(userId: string) {
  ...
  const [completedSessions, answered, correct, duration] = await Promise.all([
    ...
    prisma.practiceAnswer.count({ where: { courseId: RADIO_COURSE_ID, session: where } }),
    prisma.practiceAnswer.count({ where: { courseId: RADIO_COURSE_ID, isCorrect: true, session: where } }),
    prisma.$queryRaw<Array<{ minutes: Numeric }>>(Prisma.sql`SELECT CAST(COALESCE(SUM(TIMESTAMPDIFF(SECOND, \`startedAt\`, \`completedAt\`)), 0) AS SIGNED) / 60 AS minutes FROM \`PracticeSession\` WHERE \`courseId\` = ${RADIO_COURSE_ID} AND \`userId\` = ${userId} AND \`status\` = 'COMPLETED'`),
  ]);
...
export async function getTeacherLearningStatistics(since: Date): Promise<LearningStatistics> {
  ...
  const [completedSessions, answered, correct, activeRows, knowledgeRows, studentRows] = await Promise.all([
    ...
    prisma.practiceAnswer.count({ where: { courseId: RADIO_COURSE_ID, session: completedWhere } }),
    prisma.practiceAnswer.count({ where: { courseId: RADIO_COURSE_ID, isCorrect: true, session: completedWhere } }),
    prisma.$queryRaw<Array<{ count: Numeric }>>(Prisma.sql`SELECT CAST(COUNT(DISTINCT ps.\`userId\`) AS SIGNED) AS count FROM \`PracticeSession\` ps JOIN \`User\` u ON u.id = ps.\`userId\` WHERE ps.\`courseId\` = ${RADIO_COURSE_ID} AND ps.\`startedAt\` >= ${since} AND u.\`role\` = 'STUDENT'`),
    prisma.$queryRaw<KnowledgePointRow[]>(Prisma.sql`SELECT kp.code, kp.name, CAST(COUNT(pa.id) AS SIGNED) AS answered, CAST(SUM(CASE WHEN pa.\`isCorrect\` = TRUE THEN 1 ELSE 0 END) AS SIGNED) AS correct FROM \`PracticeAnswer\` pa JOIN \`PracticeSession\` ps ON ps.id = pa.\`sessionId\` AND ps.\`courseId\` = pa.\`courseId\` JOIN \`Question\` q ON q.id = pa.\`questionId\` AND q.\`courseId\` = pa.\`courseId\` JOIN \`KnowledgePoint\` kp ON kp.id = q.\`knowledgePointId\` AND kp.\`courseId\` = q.\`courseId\` WHERE pa.\`courseId\` = ${RADIO_COURSE_ID} AND ps.\`status\` = 'COMPLETED' AND ps.\`completedAt\` >= ${since} GROUP BY kp.id, kp.code, kp.name ORDER BY (COUNT(pa.id) - SUM(CASE WHEN pa.\`isCorrect\` = TRUE THEN 1 ELSE 0 END)) DESC, COUNT(pa.id) DESC LIMIT 10`),
    prisma.$queryRaw<StudentRow[]>(Prisma.sql`SELECT u.\`displayName\`, CAST(COUNT(DISTINCT ps.id) AS SIGNED) AS completedSessions, CAST(COUNT(pa.id) AS SIGNED) AS answered, CAST(SUM(CASE WHEN pa.\`isCorrect\` = TRUE THEN 1 ELSE 0 END) AS SIGNED) AS correct FROM \`User\` u JOIN \`PracticeSession\` ps ON ps.\`userId\` = u.id AND ps.\`courseId\` = ${RADIO_COURSE_ID} AND ps.\`status\` = 'COMPLETED' AND ps.\`completedAt\` >= ${since} LEFT JOIN \`PracticeAnswer\` pa ON pa.\`sessionId\` = ps.id AND pa.\`courseId\` = ps.\`courseId\` WHERE u.role = 'STUDENT' GROUP BY u.id, u.\`displayName\` ORDER BY COUNT(pa.id) DESC, u.\`displayName\` ASC LIMIT 20`),
  ]);
  return {
