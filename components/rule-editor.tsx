...
export function RuleEditor({ levels, points, questions, initialLevelRules, initialKnowledgeRules }: { levels: Level[]; points: KnowledgePoint[]; questions: Question[]; initialLevelRules: Record<string, PracticeRule>; initialKnowledgeRules: Record<string, PracticeRule> }) {
  ...
  return <><div className="mb-5 inline-flex rounded-xl bg-[var(--muted)] p-1"><button className={`rounded-lg px-4 py-2 text-sm font-bold ${mode === "level" ? "bg-white text-[var(--primary)] shadow-sm" : "text-[var(--muted-foreground)]"}`} onClick={() => setMode("level")}>等级综合规则</button><button className={`rounded-lg px-4 py-2 text-sm font-bold ${mode === "knowledge" ? "bg-white text-[var(--primary)] shadow-sm" : "text-[var(--muted-foreground)]"}`} onClick={() => setMode("knowledge")}>知识点专项规则</button></div>{mode === "level" ? <div className="grid gap-4 lg:grid-cols-3">{levels.map((level) => { const rule = levelRules[level.id]; const inventory = getInventory(questions, level.id); return <RuleCard key={level.id} title={level.name} subtitle="全知识点随机抽取" rule={rule} inventory={inventory} onChange={(next) => setLevelRules((current) => ({ ...current, [level.id]: next }))} />; })}</div> : <Card><CardHeader><CardTitle>知识点 + 等级专项配置</CardTitle><CardDescription>未配置的组合不会在学生端展示。单选和多选不能同时为 0。</CardDescription></CardHeader><CardContent className="overflow-x-auto p-0"><table className="min-w-[900px] w-full"><thead><tr className="border-y border-[var(--border)] bg-[var(--muted)] text-left text-xs text-[var(--muted-foreground)]"><Th>知识点</Th>{levels.map((level) => <Th key={level.id}>{level.code}级题量</Th>)}<Th>状态</Th></tr></thead><tbody>{leaves.map((point) => <tr key={point.id} className="border-b border-[var(--border)]"><Td><div className="font-extrabold">{point.code}</div><div className="mt-1 text-xs text-[var(--muted-foreground)]">{point.name}</div></Td>{levels.map((level) => { const key = `${point.id}:${level.id}`; const rule = knowledgeRules[key] ?? { singleCount: 0, multipleCount: 0 }; return <Td key={level.id}><div className="flex items-center gap-2"><MiniInput label="单" value={rule.singleCount} onChange={(value) => setKnowledgeRules((current) => ({ ...current, [key]: { ...rule, singleCount: value } }))} /><MiniInput label="多" value={rule.multipleCount} onChange={(value) => setKnowledgeRules((current) => ({ ...current, [key]: { ...rule, multipleCount: value } }))} /></div></Td>; })}<Td><Badge tone="green">可配置</Badge></Td></tr>)}</tbody></table></CardContent></Card>}{error ? <div className="mt-5 rounded-xl bg-rose-50 px-4 py-3 text-sm font-semibold text-rose-700">{error}</div> : null}<div className="mt-6 flex justify-end"><Button onClick={save} disabled={pending}>{saved ? <Check className="size-4" /> : <Save className="size-4" />}{pending ? "保存中…" : saved ? "已保存到数据库" : "保存规则"}</Button></div></>;
}
