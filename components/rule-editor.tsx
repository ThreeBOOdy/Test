...
function getInventory(questions: Question[], levelId: string): PracticeRule { const eligible = questions.filter((question) => question.levelIds.includes(levelId) && question.status === "ACTIVE"); return { singleCount: eligible.filter((question) => question.type === "SINGLE_CHOICE").length, multipleCount: eligible.filter((question) => question.type === "MULTIPLE_CHOICE").length }; }
