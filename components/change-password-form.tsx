...
export function ChangePasswordForm({ role }: { role: "STUDENT" | "TEACHER" | "ADMIN" }) {
  ...
  return <form onSubmit={submit} className="mt-7 flex flex-col gap-4"><PasswordField label="当前密码" value={currentPassword} onChange={setCurrentPassword} autoComplete="current-password" disabled={!ready || pending} /><PasswordField label="新密码" value={newPassword} onChange={setNewPassword} autoComplete="new-password" disabled={!ready || pending} /><PasswordField label="确认新密码" value={confirmation} onChange={setConfirmation} autoComplete="new-password" disabled={!ready || pending} />{message ? <div role="alert" className="rounded-xl border border-rose-300/20 bg-rose-400/10 px-4 py-3 text-sm font-semibold text-rose-200">{message}</div> : null}<Button type="submit" size="lg" disabled={!ready || pending} className="w-full"><ShieldCheck className="size-4" /><span className="min-w-24">{pending ? "保存中…" : "保存新密码"}</span></Button><div className="rounded-xl border border-[var(--border)] bg-[var(--surface-soft)] p-3 text-xs leading-6 text-[var(--muted-foreground)]">密码至少 8 位。建议包含大小写字母、数字和符号，不要与其他网站共用。</div></form>;
}

function PasswordField({ label, value, onChange, autoComplete, disabled }: { label: string; value: string; onChange: (value: string) => void; autoComplete: string; disabled: boolean }) { return <label><span className="mb-2 block text-sm font-bold">{label}</span><span className="flex h-12 items-center gap-3 rounded-xl border border-[var(--border)] bg-[var(--surface-soft)] px-4 transition-colors focus-within:border-[var(--border-strong)] focus-within:ring-2 focus-within:ring-[var(--ring)]/20"><KeyRound className="size-4 text-[var(--muted-foreground)]" /><input type="password" value={value} onChange={(event) => onChange(event.target.value)} className="w-full bg-transparent outline-none disabled:cursor-wait" minLength={8} maxLength={128} autoComplete={autoComplete} disabled={disabled} /></span></label>; }
