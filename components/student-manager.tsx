...
export function StudentManager({ students }: { students: StudentRow[] }) {
  ...
  return <>
    ...
    {form ? <Modal title={form.id ? "编辑学生" : "创建学生"} onClose={() => setForm(null)}><form onSubmit={save} className="flex flex-col gap-5"><Field label="用户名"><input value={form.username} onChange={(event) => setForm({ ...form, username: event.target.value })} className={`${inputClass} ${form.id ? "bg-[var(--muted)] text-[var(--muted-foreground)]" : ""}`} disabled={Boolean(form.id)} placeholder="字母、数字、点、横线或下划线" /></Field><Field label="显示姓名"><input value={form.displayName} onChange={(event) => setForm({ ...form, displayName: event.target.value })} className={inputClass} /></Field>{form.id ? <label className="flex items-center gap-3 rounded-xl bg-[var(--muted)] p-4"><input type="checkbox" checked={form.enabled} onChange={(event) => setForm({ ...form, enabled: event.target.checked })} className="size-4" /><span className="text-sm font-bold">启用该学生账号</span></label> : <Field label="初始密码"><input type="password" value={form.password} onChange={(event) => setForm({ ...form, password: event.target.value })} className={inputClass} minLength={8} autoComplete="new-password" /><span className="mt-2 block text-xs text-[var(--muted-foreground)]">至少 8 位。学生首次登录后需修改密码。</span></Field>}{message ? <div className="rounded-xl bg-rose-400/10 px-4 py-3 text-sm font-semibold text-rose-200">{message}</div> : null}<div className="flex justify-end gap-3"><Button type="button" variant="outline" onClick={() => setForm(null)}>取消</Button><Button type="submit" disabled={pending}>{pending ? "保存中…" : "保存学生"}</Button></div></form></Modal> : null}
    {temporaryPassword ? <Modal title="临时密码已生成" onClose={() => setTemporaryPassword(null)}><div className="rounded-2xl bg-amber-400/10 p-4 text-sm leading-6 text-amber-900">请立即将临时密码安全地交给 {temporaryPassword.name}。关闭后系统不会再次显示该明文密码。</div><div className="mt-4 flex items-center gap-3 rounded-xl border border-[var(--border)] p-4"><code className="min-w-0 flex-1 break-all text-lg font-bold">{temporaryPassword.value}</code><Button type="button" variant="outline" onClick={copyPassword}>{copied ? <Check className="size-4" /> : <Copy className="size-4" />}{copied ? "已复制" : "复制"}</Button></div><div className="mt-5 flex justify-end"><Button type="button" onClick={() => setTemporaryPassword(null)}>我已保存</Button></div></Modal> : null}
  </>;
}
