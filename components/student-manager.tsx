...
export function StudentManager({ students }: { students: StudentRow[] }) {
  ...
  return <>
    ...
    {form ? <Modal title={form.id ? "编辑学生" : "创建学生"} onClose={() => setForm(null)}><form onSubmit={save} className="flex flex-col gap-5"><Field label="用户名"><input value={form.username} onChange={(event) => setForm({ ...form, username: event.target.value })} className={`${inputClass} ${form.id ? "bg-[var(--muted)] text-[var(--muted-foreground)]" : ""}`} disabled={Boolean(form.id)} placeholder="字母、数字、点、横线或下划线" /></Field><Field label="显示姓名"><input value={form.displayName} onChange={(event) => setForm({ ...form, displayName: event.target.value })} className={inputClass} /></Field>{form.id ? <label className="flex items-center gap-3 rounded-xl bg-[var(--muted)] p-4"><input type="checkbox" checked={form.enabled} onChange={(event) => setForm({ ...form, enabled: event.target.checked })} className="size-4" /><span className="text-sm font-bold">启用该学生账号</span></label> : <Field label="初始密码"><input type="password" value={form.password} onChange={(event) => setForm({ ...form, password: event.target.value })} className={inputClass} minLength={10} maxLength={128} autoComplete="new-password" /><span className="mt-2 block text-xs text-[var(--muted-foreground)]">至少 10 位并同时包含字母和数字。学生首次登录后需修改密码。</span></Field>}{message ? <div className="rounded-xl bg-rose-50 px-4 py-3 text-sm font-semibold text-rose-700">{message}</div> : null}<div className="flex justify-end gap-3"><Button type="button" variant="outline" onClick={() => setForm(null)}>取消</Button><Button type="submit" disabled={pending}>{pending ? "保存中…" : "保存学生"}</Button></div></form></Modal> : null}
