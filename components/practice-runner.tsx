...
export function PracticeRunner({ session }: { session: Session }) {
  ...
  const [showSummary, setShowSummary] = useState(() => Object.keys(session.initialResults).length === session.questions.length);
  ...
  function move(direction: number) {
    if (direction > 0 && completed && index === session.questions.length - 1) {
      setShowSummary(true);
      return;
    }
  ...
  if (showSummary) return <ResultSummary title={session.title} correct={correctCount} total={session.total} />;
