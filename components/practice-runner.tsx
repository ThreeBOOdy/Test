...
export function PracticeRunner({ session }: { session: PublicPracticeSession }) {
  ...
  useEffect(() => {
    function handleKeyDown(event: KeyboardEvent) {
      ...
      if (event.key === "Enter") {
        ...
        if (result) moveTo(index + 1); else void submit();
        return;
      }
  ...
  }, [index, moveTo, question.options, result, submit, toggleOption]);

  if (completed) return <PracticeSummary title={session.title} correct={correctCount} total={session.total} />;
