...
export function PracticeRunner({ session }: { session: PublicPracticeSession }) {
  ...
  if (summaryVisible) {
    const wrongQuestions = session.questions
      ...
      .map((question) => ({ id: question.id, stem: question.stem }));
    return <PracticeSummary title={studentFacingTitle} correct={isExam ? examResult?.correctCount ?? 0 : correctCount} total={isExam ? examResult?.total ?? session.total : session.total} passingCount={isExam ? examResult?.passingCount : session.exam?.passingCount} settlementSource={isExam ? examResult?.settlementSource : undefined} wrongQuestions={wrongQuestions} sessionId={session.id} boss={isExam && examResult ? { correct: examResult.correctCount, total: examResult.total, passingCount: examResult.passingCount } : undefined} />;
  }
  ...
  return <div className="practice-reading safe-bottom mx-auto max-w-7xl">
    ...
    {session.mode === "RANDOM_ALL" && session.stageCompleted === true ? <div className="mb-5 flex items-start gap-3 rounded-2xl border border-emerald-300/20 bg-emerald-400/10 px-4 py-3 text-sm font-semibold text-emerald-100"><CheckCircle2 className="mt-0.5 size-5 shrink-0" /><div><div className="font-bold">阶段性完成</div><div className="mt-1 text-sm leading-6">所有题目已进入长期复习，仍可继续随机复习巩固掌握。</div></div></div> : null}
