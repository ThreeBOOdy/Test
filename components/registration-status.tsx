...
export function RegistrationStatus({ initialData, grades }: { initialData: RegistrationStatusData; grades: GradeOption[] }) {
  ...
  async function save(event: React.FormEvent) {
    ...
    try {
      ...
      setData({ ...data, displayName: form.displayName.trim(), school: form.school.trim(), grade, gender: derivedGender ?? form.gender });
      setForm(null);
  ...
  return <div className="mx-auto min-h-screen max-w-4xl px-4 py-8 sm:px-8">
    ...
    <Card className="mt-6"><CardContent className="grid gap-4 sm:grid-cols-2"><Info label="用户名" value={data.username} /><Info label="姓名" value={data.displayName} /><Info label="身份证号" value={data.nationalIdMasked} /><Info label="性别" value={data.gender === "MALE" ? "男" : "女"} /><Info label="学校" value={data.school} /><Info label="年级" value={data.grade.name} /><Info label="手机号" value={data.phoneMasked} /><Info label="提交时间" value={data.submittedAt ? new Date(data.submittedAt).toLocaleString("zh-CN") : "—"} /></CardContent></Card>
