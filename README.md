...
- 自主注册时从无线电人物目录选择独立登录用户名；人物身份确认后永久绑定，管理员不能修改。
...
普通题目默认随机打乱选项顺序，`preserveOptionOrder` 标记的题目保持原顺序并在导入时提示题干依赖字母或位置的风险；题目与选项顺序在会话创建时写入快照并冻结。
...
- Vitest 单元、UI 与仓库规则测试：61 个测试文件，358 项通过、1 项跳过。
- MySQL 集成测试：6 个文件、58 项通过，覆盖课程迁移、学生注册、导入、审核、敏感数据、题库、练习、考试与清理。
- Playwright 端到端测试：3/3 通过（管理员导入并激活学生、练习断网恢复与错题三会话掌握、教师 Excel 批次预检/报告/提交/撤销）。
...
当前统一主线：`main`；本轮分票实现与验收位于 `codex/unified-practice-and-multisheet-import` 分支（已推送）。
